...
// NewBatchService creates a new product batch service.
...
// GetByProductID retrieves all batches for a specific product.
...
func (s *BatchService) Create(req CreateBatchRequest, user *auth.User) (*ProductBatch, error) {
...
func (s *BatchService) Update(id uuid.UUID, req UpdateBatchRequest, user *auth.User) (*ProductBatch, error) {
	if err := validator.Struct(req); err != nil {
		return nil, err
	}
